...
# with default values
model1_train = loess(data_train$v_Vel_output ~ data_train$v_Vel + data_train$v_Acc + data_train$Space_Headway)
model1_test = loess(data_test$v_Vel_output ~ data_test$v_Vel + data_test$v_Acc + data_test$Space_Headway)
...
rmsn1_train = rmse1_train/mean(v_true_train) #0.1410644
rmse1_test = sqrt(mean((v_true_test - pred1_test)^2))
rmsn1_test = rmse1_test/mean(v_true_test) #0.07752121


# with optimized values
model2_train = loess_wrapper_extrapolate(data_train$v_Vel + data_train$v_Acc + data_train$Space_Headway, data_train$v_Vel_output)

model2_test = loess(data_test$v_Vel_output ~ data_test$v_Vel + data_test$v_Acc + data_test$Space_Headway, span = 0.23, degree = 2, control = loess.control(surface = "direct"))
...
rmsn2_train = rmse2_train/mean(data_train$v_Vel_output) #0.2834495
rmse2_test = sqrt(mean((v_true_test - pred2_test)^2))
rmsn2_test = rmse2_test/mean(data_test$v_Vel_output) #0.0643018


# plot the result
library(ggplot2)
tiff("3 inputs_acc+space.png", units = "in", width = 20, height = 8, res = 300)
fig1 = ggplot()
fig1 = fig1 + geom_point(data = comp1_train, aes(x = data_train.Frame_ID, y = data_train.v_Vel_output), size = 0.8)
...
fig1 = fig1 + geom_line(data = comp1_test, aes(x = data_test.Frame_ID, y = pred1_test), color = "blue") 
fig1 = fig1 + geom_line(data = comp2_test, aes(x = data_test.Frame_ID, y = pred2_test), color = "red")
