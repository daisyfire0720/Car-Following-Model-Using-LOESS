...
# with default values
model1_train = loess(data_train$v_Vel_output ~ data_train$v_Vel + data_train$v_Acc + data_train$Space_Headway + data_train$Time_Headway)
model1_test = loess(data_test$v_Vel_output ~ data_test$v_Vel + data_test$v_Acc + data_test$Space_Headway + data_test$Time_Headway)
...
rmsn1_train = rmse1_train/mean(v_true_train) #1.586967
rmse1_test = sqrt(mean((v_true_test - pred1_test)^2))
rmsn1_test = rmse1_test/mean(v_true_test) #0.464768


# with optimized values
model2_train = loess_wrapper_extrapolate(data_train$v_Vel + data_train$v_Acc + data_train$Space_Headway + data_train$Time_Headway, data_train$v_Vel_output)

model2_test = loess(data_test$v_Vel_output ~ data_test$v_Vel + data_test$v_Acc + data_test$Space_Headway + data_test$Time_Headway, span = 0.13, degree = 2, control = loess.control(surface = "direct"))
...
rmsn2_train = rmse2_train/mean(data_train$v_Vel_output) #0.3266716
rmse2_test = sqrt(mean((v_true_test - pred2_test)^2))
rmsn2_test = rmse2_test/mean(data_test$v_Vel_output) #0.04443268


# plot the result
library(ggplot2)
tiff("4 inputs.png", units = "in", width = 20, height = 8, res = 300)
fig1 = ggplot()
